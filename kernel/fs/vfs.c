...
static mount_t *find_mount(const char *path)
{
    ...
    for (int i = 0; i < mount_count; i++) {
        ...
        if (match && mounts[i].path_len > best_len) {
            if (path[mounts[i].path_len] == '/' || path[mounts[i].path_len] == 0) {
                best = &mounts[i];
...
int vfs_open(const char *path, int flags)
{
    ...
    const char *sub = strip_mount(path, m);
    ...
    for (int i = 0; i < VFS_MAX_FDS; i++) {
        if (!fd_table[i].used) {
            ...
            int j = 0;
            while (path[j] && j < VFS_NAME_LEN - 1) { fd_table[i].path[j] = path[j]; j++; }
            fd_table[i].path[j] = 0;
            fd_table[i].offset = 0;
...
int vfs_readdir(const char *path, vfs_entry_t *entries, int max)
{
    ...
    return m->ops->readdir(strip_mount(path, m), entries, max);
}
...
int vfs_mkdir(const char *path, uint32_t mode)
{
    ...
    return m->ops->mkdir(strip_mount(path, m), mode);
}
...
int vfs_unlink(const char *path)
{
    ...
    return m->ops->unlink(strip_mount(path, m));
}
...
int vfs_stat(const char *path, vfs_entry_t *entry)
{
    ...
    return m->ops->stat(strip_mount(path, m), entry);
}

int vfs_rename(const char *old, const char *new_path)
{
    mount_t *m = find_mount(old);
    if (!m || !m->ops || !m->ops->rename) return -1;
    mount_t *m2 = find_mount(new_path);
    if (m != m2) return -1;
    return m->ops->rename(strip_mount(old, m), strip_mount(new_path, m2));
}
...
int vfs_symlink(const char *target, const char *path)
{
    ...
    return m->ops->symlink(target, strip_mount(path, m));
}
